...
from metrics import preprocess_images , calculate_inception_score , InceptionV3 , calculate_inception_score_first_version , get_FID
...
nid= "vnoise"
def test(model, device, test_loader, calculate_inception = False , Calculate_FID= False , N = 64):
    model.eval()
    test_loss = 0
    ctime = time.time()
    j, k, h = test_loader.dataset[0][0].shape[-3:]  
    collected_data = torch.empty((0, j, k, h), dtype=torch.float32, device=device)
    with torch.no_grad():
    #if True:
        for data, target in test_loader:
            ...
            if collected_data.shape[0] < N:
                collected_data = torch.cat((collected_data, im), dim=0)
            test_loss += loss.item()
    ...
    if calculate_inception :
        normalized = gen_im.to(device)
        mean,std = calculate_inception_score_first_version(normalized, batch_size=32, splits=10,dataset="MNIST")
        print(f"Inception Score: {mean} ± {std}")
    if Calculate_FID :
        get_FID(collected_data,gen_im)
...
def main():
    global sigmas
    # Training settings
    args_dict = {'batch_size' : 64, 'test_batch_size' :64, 'epochs' :200, 'lr' : 0.0002, 'gamma' : 0.98, 'no_cuda' :False, 'dry_run':False, 'seed': 1, 'log_interval' : 200, 'save_model' :True, 'only_test':True, 'model_path':"denoisermnist.pt", 'load_model_from_disk':False}
    if TEST:
